...
/// Convert provider-agnostic attachment blocks to OpenAI/Grok/OpenRouter content parts.
///
/// OpenAI format:
/// - Images: `{"type":"image_url","image_url":{"url":"data:image/jpeg;base64,..."}}`
/// - Files: `{"type":"file","file":{"file_data":"data:...;base64,...","filename":"..."}}`
/// - Text: `{"type":"text","text":"--- filename ---\n<content>"}`
pub fn convert_for_openai(attachments: &[Value]) -> Vec<Value> {
    let mut blocks = Vec::new();
    for att in attachments {
        let media_type = att.get("media_type").and_then(|v| v.as_str()).unwrap_or("");
        let data = att.get("data").and_then(|v| v.as_str()).unwrap_or("");
...
/// Convert provider-agnostic attachment blocks to Gemini parts.
///
/// Gemini format:
/// - Images/PDFs/binary: `{"inlineData":{"mimeType":"image/jpeg","data":"..."}}`
/// - Text: `{"text":"--- filename ---\n<content>"}`
pub fn convert_for_gemini(attachments: &[Value]) -> Vec<Value> {
