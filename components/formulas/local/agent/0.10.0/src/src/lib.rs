...
fn handle_request(input: &str) -> Result<String, String> {
    ...
    let mut conversation = build_initial_messages(&parsed, task, provider, &attachments)?;
    ...
    if !attachments.is_empty() {
        strip_attachment_data(&mut conversation, provider);
    }
...
fn build_user_message_with_attachments(
    ...
    match provider {
        ...
        Provider::OpenAI => {
            // OpenAI uses "content" array of typed parts
...
fn build_openai_request(
    ...
    // Web search requires using the Responses API or a search-specific model with
    // web_search_options param — neither works through our catalyst's messages.create path.
    // OpenRouter also does not support native search tools in the tools array.
    if !all_tools.is_empty() {
        params["tools"] = json!(all_tools);
    }
