...
No API keys or secrets required. Domain access is controlled by the host policy (`allowed_domains` in the manifest `setup.policy`).
...
This reads the manifest and applies the default host policy (all domains allowed).
...
```bash
cyfr register
cyfr policy set c:local.web allowed_domains '["example.com", "*.wikipedia.org"]'
```
...
cyfr run c:local.web --input '{"operation": "extract", "params": {"url": "https://example.com"}}'

# Raw HTTP fetch
cyfr run c:local.web --input '{"operation": "fetch", "params": {"url": "https://example.com"}}'

# Discover links on a page
cyfr run c:local.web --input '{"operation": "links", "params": {"url": "https://example.com"}}'
