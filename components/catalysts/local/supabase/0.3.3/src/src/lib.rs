...
/// Send an HTTP request with a base64-encoded binary body.
/// The CYFR runtime decodes the base64 to raw bytes before sending.
pub(crate) fn do_request_binary(method: &str, url: &str, headers: &Value, body_base64: &str) -> String {
    let req = json!({
        "method": method,
        "url": url,
        "headers": headers,
...
fn realtime_broadcast(ctx: &SupabaseContext, params: &Value) -> Result<String, String> {
    ...
    let headers = build_headers(&ctx.publishable_key, if extra.is_empty() { None } else { Some(&extra) });
    let body = json!({
        "messages": [{
