...
use crate::{build_headers, do_request, do_request_binary, parse_response, require_param, SupabaseContext};
...
pub fn upload(ctx: &SupabaseContext, params: &Value) -> Result<String, String> {
    ...
    let encoding = params.get("body_encoding").and_then(|v| v.as_str());
    if encoding == Some("base64") {
        Ok(parse_response(&do_request_binary("POST", &url, &headers, body)))
    } else {
