...
impl SupabaseContext {
    fn load() -> Result<Self, String> {
        ...
        let anon_key = read::get("SUPABASE_ANON_KEY")
            .map_err(|e| format!("Failed to read SUPABASE_ANON_KEY: {e}"))?;

        // Trim trailing slash from URL
        let url = url.trim_end_matches('/').to_string();

        let service_role_key = read::get("SUPABASE_SERVICE_ROLE_KEY").ok();
    ...
    /// Pick the API key: service role if requested and available, otherwise anon.
